...
class eyelink_start_recording(item.item):
	...
	def __init__(self, name, experiment, string = None):
		...
		self.item_type = "eyelink_start_recording"		
		
		self.xpos = 0
		self.ypos = 0
		
		# Provide a short accurate description of the items functionality
		self.description = "Start recording plugin for the Eyelink series of eye trackers (SR-Research)"

		# The parent handles the rest of the contruction
		item.item.__init__(self, name, experiment, string)
		
		if not hasattr(self, "log_msg"):
			self.log_msg = "start_trial"
	...
	def run(self):
		...
		self.experiment.eyelink.start_recording()
		self.experiment.eyelink.status_msg(self.eval_text(self.get("log_msg")))
		self.experiment.eyelink.log(self.eval_text(self.get("log_msg")))
				
		# Report success
		return True
					
class qteyelink_start_recording(eyelink_start_recording, qtplugin.qtplugin):
	...
	def __init__(self, name, experiment, string = None):
		...
		eyelink_start_recording.__init__(self, name, experiment, string)		
	...
	def init_edit_widget(self):
		...
		self.add_line_edit_control("log_msg", "Log message", default = "start_trial", tooltip = "A message to write to the eyelink logfile.", min_width = 400)
