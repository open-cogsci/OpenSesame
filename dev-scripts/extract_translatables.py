...
# A regular expression that (purportedly) detects all Python strings in the
# code. Source:
...
def detect_translatables_yaml(path, context):
    ...
    for field in [u'label', u'description', u'tooltip', u'category']:
        if field in d:
            if context not in translatables:
                translatables[context] = []
            translatables[context].append(
                'u"%s"' % (d[field].replace(u'"', u'\\"'))
            )
    if u'controls' in d:
...
def encode_translatables(path=u'dev-scripts/translatables.py'):
    ...
    """
    desc:
        Writes all translatables to a Python script, which can be interpreted
        by pylupdate4.
...
def parse_folder(path, context=None, translate=True, py=True, yaml=True):
    ...
    keywords:
        ...
        py:
            ...
            type:	bool
        yaml:
            desc:	Indicates whether .yaml / .json  files should be
                    processed.
            type:	bool
    """
    ...
    for f in os.listdir(path):
        if context is not None:
            _context = context
        elif path in [u'opensesame_extensions', u'opensesame_plugins']:
            _context = f
        else:
            _context = u'script'
...
if __name__ == u'__main__':
    ...
    parse_folder('opensesame_extensions')
    parse_folder('opensesame_plugins', py=False)
