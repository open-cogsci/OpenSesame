...
class eyelink_stop_recording(item.item):
	...
	def __init__(self, name, experiment, string = None):
		...
		self.item_type = "eyelink_stop_recording"		
		
		self.xpos = 0
		self.ypos = 0
		
		# Provide a short accurate description of the items functionality
		self.description = "Stop recording plugin for the Eyelink series of eye trackers (SR-Research)"
	...
	def run(self):
		...
		self.experiment.eyelink.stop_recording()
				
		# Report success
		return True
					
class qteyelink_stop_recording(eyelink_stop_recording, qtplugin.qtplugin):
	...
	def __init__(self, name, experiment, string = None):
		...
		eyelink_stop_recording.__init__(self, name, experiment, string)		
	...
	def init_edit_widget(self):
		...
		self.add_line_edit_control("log_msg", "Log message", default = "stop_trial", tooltip = "A message to write to the eyelink logfile.", min_width = 400)
