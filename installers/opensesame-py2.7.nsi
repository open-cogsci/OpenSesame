...
!define PRODUCT_VERSION "3.2.3-py2.7-win32-1"
...
Section "OpenSesame" SEC01
  ...
  true:
    ...
    ${registerExtension} "$INSTDIR\${EXEC_SUBFOLDER}opensesame.exe" ".osexp" "OpenSesame experiment"
SectionEnd

Section -AdditionalIcons
  WriteIniStr "$INSTDIR\${PRODUCT_NAME}.url" "InternetShortcut" "URL" "${PRODUCT_WEB_SITE}"
  CreateDirectory "$SMPROGRAMS\OpenSesame"
  CreateShortCut "$SMPROGRAMS\OpenSesame\OpenSesame.lnk" "$INSTDIR\pythonw.exe" "${EXEC_SUBFOLDER}safelaunch-opensesame.py" "$INSTDIR\Lib\site-packages\share\opensesame_resources\opensesame.ico"
  CreateShortCut "$SMPROGRAMS\OpenSesame\OpenSesame (runtime).lnk" "$INSTDIR\pythonw.exe" "${EXEC_SUBFOLDER}safelaunch-opensesamerun.py" "$INSTDIR\Lib\site-packages\share\opensesame_resources\opensesamerun.ico"
